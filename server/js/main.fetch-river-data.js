...
const NUM_RIVER_DATUM_TO_KEEP = 10;

const fetchRiverDataFromUSGS = () => {
  RiverInstruments.find().forEach(inst => {
    [RiverInstrument.GAGE_HEIGHT_CODE, RiverInstrument.DISCHARGE_CODE].forEach(paramCode => {
      ...
      if (200 === reply.statusCode) {
        try {
          ...
          if (RiverData.find({ timestamp: new Date(valueObject.dateTime), paramCode: paramCode }).count() === 0) {
            new RiverDatum({
